...
class DataParallelPPOActor(BasePPOActor):
    ...
    def update_policy(self, data: DataProto) -> dict[str, Any]:
        ...
        # Include token_weights if present
        if "token_weights" in data.batch:
            select_keys.append("token_weights")
        non_tensor_select_keys = ["multi_modal_inputs"]
        ...
        for _ in range(self.config.ppo_epochs):
            ...
            for mini_batch in mini_batches:
                ...
                for micro_batch in micro_batches:
                    ...
                    # Extract token_weights if present and apply to response_mask
                    token_weights = model_inputs.get("token_weights")
                    if token_weights is not None:
                        # Align token_weights to response_mask shape
                        response_length = response_mask.size(-1)
                        if token_weights.size(-1) < response_length:
                            # Pad with ones
                            pad_size = response_length - token_weights.size(-1)
                        ...
                        # Apply token weights to response mask
