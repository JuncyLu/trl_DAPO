...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        self._token_weight_warning_emitted = False
    ...
    def _maybe_compute_token_weights(self, batch: DataProto, reward_extra_infos: dict) -> Optional[torch.Tensor]:
        """Compute VGR-based token-level weights if enabled in config."""
        if not self.config.algorithm.enable_token_weights:
            return None
        
        # Try to get from reward extra infos first
        token_weights = reward_extra_infos.get("token_weights")
        if token_weights is not None:
            if isinstance(token_weights, list):
                # Convert list of lists to tensor
                max_len = max(len(w) for w in token_weights)
                weight_tensor = torch.ones(len(token_weights), max_len, dtype=torch.float32)
                for i, w in enumerate(token_weights):
                    if len(w) > 0:
                        weight_tensor[i, :len(w)] = torch.tensor(w, dtype=torch.float32)
                return weight_tensor
            elif isinstance(token_weights, torch.Tensor):
                return token_weights
        
        # If not in reward extra infos, emit warning once
        if not self._token_weight_warning_emitted:
            print("Warning: token_weights enabled but not found in reward extra infos. Token weights will not be applied.")
            self._token_weight_warning_emitted = True
            ...
            with timer("step", timing_raw):
                ...
                with timer("adv", timing_raw):
                    if "token_level_scores" not in batch.batch:
                        ...
                        # Compute and add token weights if enabled
