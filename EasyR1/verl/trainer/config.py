...
class AlgorithmConfig:
    ...
    """enable VGR-based token-level weights for actor loss"""
    token_weight_clip: Tuple[float, float] = (0.2, 3.0)
    """clip range for token weights"""
    token_weight_smooth_sigma: float = 0.0
    """gaussian smoothing sigma for token weights, 0.0 means no smoothing"""
    token_weight_last_k_layers: Optional[int] = None
    """use only last K layers for attention computation, None means all layers"""
